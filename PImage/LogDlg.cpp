...
BOOL CLogDlg::OnInitDialog()
{
	...
	if(m_filterType == IDM_THRESHOLD ) //阈值处理时
	{
		GetDlgItem(IDC_STATIC_LOG)->SetWindowText("请输入T0:");
	}
	else if(m_filterType == IDM_GLT_LOGARITHM )//对数变换
	{
		GetDlgItem(IDC_STATIC_LOG)->SetWindowText("请输入系数:");
	}
	else if(m_filterType == IDM_FIELD_GROW )//区域生长
	{
		GetDlgItem(IDC_STATIC_LOG)->SetWindowText("请输入阈值系数:");
