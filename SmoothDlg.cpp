...
BOOL CSmoothDlg::OnInitDialog()
{
	...
	{
		GetDlgItem(IDC_RADIO7)->ShowWindow(FALSE); //隐藏该控件;
	}
	creatEdit(m_radioGroup);//初始化都只设置一个模板
	return true;
}
...
void CSmoothDlg::OnBnClickedRadio3()
{
	...
	case MODE_ZERO:
		OnClose(m_MyEdits);
		creatEdit(MODE_ZERO);//初始化都只设置一个模板
		break;
	case MODE_ONE:
		OnClose(m_MyEdits);
		creatEdit(MODE_ONE);//初始化都只设置一个模板
		break;
	case MODE_TWO:
		OnClose(m_MyEdits);
		creatEdit(MODE_TWO);//初始化都只设置一个模板
	...
	for(int i =0; i<m_modelMartrix; i++)
	{
		for(int j =0; j<m_modelMartrix; j++)
		{
			...
			else if(m_filterType ==IDM_EDGE_LOG)
			{
				...
				if(m_modelMartrix == 3)
				{
					value = j+(i*m_modelMartrix); //得到 坐标
					if(value%2 != 0 )
					{
						m_staticCaption[value].Format("%d",1);
					}
